...
def post_detail(request, slug):
    ...
    new_comment_submitted = (
        request.method == 'POST'
        and (
            request.POST.get('action') == 'add_comment'
    ...
    if new_comment_submitted:
        ...
        if content:
            ...
            session_id = (
                request.POST.get('anon_session_id')
                or request.POST.get('session_id')
                or None
            )

            # Determine approval rules for registered users:
            # - If user has a previously approved comment, auto-approve this one.
            # - Otherwise (first-time commenter), mark as not approved and notify them.
            approved = False
            if author:
                has_approved_before = Comment.objects.filter(
                    author=author,
                    approved=True,
                ).exists()
                approved = bool(has_approved_before)
            ...
            if not author:
                messages.success(
                    request,
                    (
                        'Thanks — your comment was submitted and will be displayed '
                        'once approved by an administrator.'
                    ),
                )

            # For registered users submitting a first comment (not yet approved)
            if author and not approved:
                messages.success(
                    request,
                    (
                        'Thanks — your comment was submitted and will be displayed '
                        'once approved by an administrator. Future comments will be '
                        'approved automatically.'
                    ),
                )

            # If this is a registered, already-approved commenter and the comment is
            # auto-approved, support AJAX by returning rendered HTML for insertion.
        ...
        if not request.user.is_authenticated and session_id:
            anon_redirect = (
                f"{post.get_absolute_url()}?anon_sess={session_id}#messages"
            )
            return redirect(anon_redirect)
    ...
    if request.user.is_authenticated:
        comments = (
            post.comments.filter(Q(approved=True) | Q(author=request.user))
            .order_by('-created_at')
        )
    else:
